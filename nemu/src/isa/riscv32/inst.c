...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = Mr(src1 + imm, 2));

  //INSTPAT("??????? ????? ????? 010 ????? 01000 11", sb     , S, Mw(src1 + imm, 4, src2));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, printf("%x %x %x\n", src1, imm, imm & 0x3f);R(dest) = src1 >> (imm & 0x3f));//
