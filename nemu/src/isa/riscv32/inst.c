...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(dest) = src1 << (imm & 0x3f)); 
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, imm = imm & 0x3f; R(dest) = src1 >> imm ); //TODO:?
  //INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, imm = imm & 0x3f; R(dest) = (src1 >> imm) & ~((1 << 31) >> imm << 1)); //TODO:?
