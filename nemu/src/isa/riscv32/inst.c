...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->pc + 4 ;s->dnpc = s->pc + imm;  check_func_log(s->dnpc, s->pc) );
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (src1 + imm) & ~1; R(dest) = s->pc + 4; check_func_log(s->dnpc, s->pc)) ;
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(dest) = SEXT(Mr(src1 + imm, 1), 24)); //
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(dest) = src1 << (imm & 0x3f)); //0x3f?
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(dest) = src1 >> (imm & 0x3f)); //0x3f?
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = (int)src1 >> (imm & 0x1f)); //0x3f?
  ...
  //INSTPAT("??????? ????? ????? 101 ????? 11100 11", csrrwi , Z, R(dest) = CSR(imm); CSR(imm) = src1);//
  //INSTPAT("??????? ????? ????? 110 ????? 11100 11", csrrsi , Z, R(dest) = CSR(imm); CSR(imm) |= src1);//
  //INSTPAT("??????? ????? ????? 111 ????? 11100 11", csrrci , Z, R(dest) = CSR(imm); CSR(imm) = CSR(imm) & ~ziim);//



  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(dest) = src1 * src2);
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(dest) = (word_t)((SEXT(src1, 32) * SEXT(src2, 32)) >> 32));
  //INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(dest) = (word_t)((SEXT(src1, 32) * (int64_t)src2) >> 32)); //
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(dest) = (word_t)(((uint64_t)src1 * (uint64_t)src2) >> 32)); //
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, printf("mret set pc=%08x\n", csr.mepc); s->dnpc = csr.mepc; csr.mstatus = (csr.mstatus & ~0x8) | ((csr.mstatus >> 4) & 0x8); csr.mstatus |= 0x80);
